...
#  FIXED: Renamed 'name' to 'display_name' to avoid Resource.name conflict
...
@export var display_name: String  #  RENAMED from 'name'
...
static func create_from_dict(item_id: String, data: Dictionary) -> Item:
	var item = Item.new()
	item.id = item_id
	item.display_name = data.name  #  Load into display_name
	...
	if item.item_type == ItemType.CONSUMABLE:
		...
		if data.has("target_type"):
			item.target_type = TargetType[data.target_type]
		else:
			item.target_type = TargetType.SELF
...
func heal(user: CharacterData, targets: Array) -> String:
	...
	if is_percentage_based:
		return "%s restored %.0f%% HP (%d HP) to %d target(s)" % [
			display_name, effect_percent * 100, total_heal, targets.size()
		]
	else:
		return "%s healed %d HP to %d target(s)" % [display_name, total_heal, targets.size()]
...
func restore(user: CharacterData, targets: Array) -> String:
	...
	if is_percentage_based:
		return "%s restored %.0f%% MP/SP to %d target(s)" % [
			display_name, effect_percent * 100, targets.size()
		]
	else:
		return "%s restored %d MP to %d target(s)" % [display_name, total_restore, targets.size()]
			...
			if poison_chance < 1.0:
				if RandomManager.randf() <= poison_chance:
					...
					return "%s dealt %d damage and inflicted %s!" % [
						display_name, total_damage, Skill.StatusEffect.keys()[status_effect]
					]
			else:
				target.apply_status_effect(status_effect, effect_duration)
				return "%s dealt %d damage and inflicted %s for %d turns!" % [
					display_name, total_damage, Skill.StatusEffect.keys()[status_effect], effect_duration
				]
	
	return "%s dealt %d damage to %d target(s)" % [display_name, total_damage, targets.size()]
...
func apply_buff(user: CharacterData, targets: Array) -> String:
	print("[ITEM DEBUG] apply_buff called: %s | buff_type=%s | effect_percent=%.2f | effect_power=%d | duration=%d" % [
		display_name, buff_type, effect_percent, effect_power, effect_duration
	])
	
	var buff_value = 0
	if is_percentage_based:
		buff_value = int(effect_percent * 100)
	else:
	...
	if is_percentage_based:
		return "%s applied %s buff (+%.0f%%) to %d target(s) for %d turns" % [
			display_name, buff_type, effect_percent * 100, targets.size(), effect_duration
		]
	else:
		return "%s applied %s buff (+%d) to %d target(s) for %d turns" % [
			display_name, buff_type, buff_value, targets.size(), effect_duration
		]
...
func apply_debuff(user: CharacterData, targets: Array) -> String:
	for target in targets:
		if status_effect != Skill.StatusEffect.NONE:
			target.apply_status_effect(status_effect, effect_duration)
	return "%s applied debuff to %d target(s)" % [display_name, targets.size()]
...
func cure_status(user: CharacterData, targets: Array) -> String:
	...
	var result = ""
	if status_effect != Skill.StatusEffect.NONE:
		result = "%s cured %s" % [display_name, Skill.StatusEffect.keys()[status_effect]]
	else:
		result = "%s cured all status effects" % display_name
