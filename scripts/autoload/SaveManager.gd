...
func save_game(player: CharacterData):
	...
	for item_key in player.inventory.items:
		...
		if item is Equipment:
			...
			# CRITICAL FIX: Convert enum keys to strings for JSON
			if "stat_modifiers" in item and not item.stat_modifiers.is_empty():
				var converted_mods = {}
				for stat_enum in item.stat_modifiers:
					# Convert enum to string key
					var stat_name = Skill.AttributeTarget.keys()[stat_enum]
					converted_mods[stat_name] = item.stat_modifiers[stat_enum]
				equip_save_data["stat_modifiers"] = converted_mods
	...
	for item_key in player.stash.items:
		...
		if item is Equipment:
			...
			# CRITICAL FIX: Convert enum keys to strings
			if "stat_modifiers" in item and not item.stat_modifiers.is_empty():
				var converted_mods = {}
				for stat_enum in item.stat_modifiers:
					var stat_name = Skill.AttributeTarget.keys()[stat_enum]
	...
	for slot in player.equipment:
		if player.equipment[slot]:
			...
			var equip_data = {
				...
				"inventory_key": equipped_item.inventory_key
			}
			
			# CRITICAL FIX: Convert enum keys to strings
			if "stat_modifiers" in equipped_item and not equipped_item.stat_modifiers.is_empty():
				var converted_mods = {}
				for stat_enum in equipped_item.stat_modifiers:
					var stat_name = Skill.AttributeTarget.keys()[stat_enum]
					converted_mods[stat_name] = equipped_item.stat_modifiers[stat_enum]
				equip_data["stat_modifiers"] = converted_mods
		...
		for item_key in save_data["inventory"]:
			...
			if item_data.get("is_equipment", false):
				...
				if base_item and base_item is Equipment:
					...
					base_item.inventory_key = item_key
					
					# CRITICAL FIX: Convert string keys back to enum
					if "stat_modifiers" in item_data:
						base_item.stat_modifiers = {}
						for stat_name in item_data["stat_modifiers"]:
							if Skill.AttributeTarget.has(stat_name):
								var enum_val = Skill.AttributeTarget[stat_name]
								base_item.stat_modifiers[enum_val] = item_data["stat_modifiers"][stat_name]
		...
		for slot in save_data["equipment"]:
			...
			if typeof(equip_data) == TYPE_DICTIONARY:
				...
				if item and item is Equipment:
					...
					item.inventory_key = equip_data.get("inventory_key", item.id)
					
					# CRITICAL FIX: Convert string keys back to enum
					if "stat_modifiers" in equip_data:
						item.stat_modifiers = {}
						for stat_name in equip_data["stat_modifiers"]:
							if Skill.AttributeTarget.has(stat_name):
								var enum_val = Skill.AttributeTarget[stat_name]
								item.stat_modifiers[enum_val] = equip_data["stat_modifiers"][stat_name]
