...
func deal_damage(user: CharacterData, targets: Array):
	...
	for target in targets:
		...
		var resistance = (target.toughness if ability_type == AbilityType.PHYSICAL else target.spell_ward)
		...
		if status_effect != StatusEffect.NONE:
			target.apply_status_effect(status_effect, duration)
...
func restore(user: CharacterData, targets: Array):
	...
	for target in targets:
		var heal_amount = power + user.spell_power
		target.restore_mp(heal_amount)
		total_heal += heal_amount
	return "%s healed %d MP to %d target(s)" % [name, total_heal, targets.size()]

func inflict_status(user: CharacterData, targets: Array):
	var affected_targets = 0
	for target in targets:
		if target.apply_status_effect(status_effect, duration):
			affected_targets += 1
